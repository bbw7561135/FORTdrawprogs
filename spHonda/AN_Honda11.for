...
************************************************************************
     ...
     #                Rescale= 1.0d-04
                 REAL
     #                E(NE),C(NC),phi(Np),
     #                Faz_en(NE,Np,NC),Faz_ea(NE,Np,NC),
     #                Faz_mn(NE,Np,NC),Faz_ma(NE,Np,NC),
     ...
     #                NTn(Nt)/'n','a'/
         ...
         OPEN (Ndat00,FILE=datACN//'AN_Honda11_az.data')
         DO n_NC=0,NC-1
           DO n_Np=1,Np
     ...
     #                        Faz_ea(n_NE,n_Np,NC-n_NC)
      ...
      endDO
         ...
         Faz_mn=Faz_mn*Rescale
         Faz_ma=Faz_ma*Rescale
         Faz_en=Faz_en*Rescale
         Faz_ea=Faz_ea*Rescale
...
*        ============================================================= *
         IF (Spectra) THEN
           DO n_Nfl=1,Nfl
             DO nNTn=1,Nt
               OPEN(Ndat(n_Nfl,nNTn),FILE=outACN//dir//'FE_'//fln(n_Nfl)
     #                                                //NTn(nNTn)//ext)
          endDO
        endDO
           DO n_NE=1,NE
             WRITE(Ndat(1,1),1) E(n_NE),(F_en(n_NE,n_NC),n_NC=1,NC)
             WRITE(Ndat(1,2),1) E(n_NE),(F_ea(n_NE,n_NC),n_NC=1,NC)
             WRITE(Ndat(2,1),1) E(n_NE),(F_mn(n_NE,n_NC),n_NC=1,NC)
             WRITE(Ndat(2,2),1) E(n_NE),(F_ma(n_NE,n_NC),n_NC=1,NC)
        endDO
           DO n_Nfl=1,Nfl
             DO nNTn=1,Nt
               CLOSE(Ndat(n_Nfl,nNTn))
               OPEN(Ndat(n_Nfl,nNTn),FILE=outACN//dir//'SE_'//fln(n_Nfl)
     #                                                //NTn(nNTn)//ext)
          endDO
        endDO
     ...
     #                     (E,C,F_ma,C_ma,NE,NC,Energy,Cosine)
          endDO
             WRITE(Ndat(1,1),2) Energy,(SF_en(n_NC),n_NC=1,NC_test)
             WRITE(Ndat(1,2),2) Energy,(SF_ea(n_NC),n_NC=1,NC_test)
             WRITE(Ndat(2,1),2) Energy,(SF_mn(n_NC),n_NC=1,NC_test)
             WRITE(Ndat(2,2),2) Energy,(SF_ma(n_NC),n_NC=1,NC_test)
        endDO
           DO n_Nfl=1,Nfl
             DO nNTn=1,Nt
               CLOSE(Ndat(n_Nfl,nNTn))
