...
************************************************************************
     ...
     #                Nm_fin       =   3,                                Number of points for m_fin
     #                MinCal       = 120                                 Number of integrand calls
                 REAL,PARAMETER::
     #                RelErr       = 1.000d-04,                          Relative integration accuracy [see MuL.for]
     #                m_fin_ini    = 1.200d+00,                          Ininial W^DIS_cut
     #                m_fin_fin    = 3.000d+00,                          Final   W^DIS_cut
     #                stepm_fin    = (m_fin_fin-m_fin_ini)/(Nm_fin-1),   Step for the m_fin
     ...
     #                factor       = G_Fermi**2*hc2*1.0d+38/pi           Global factor for the cross sections
         ...
         DIMENSION E_nu_out(NE_nu),sDIS(NE_nu,Nm_fin),
     #                             qDIS(NE_nu,Nm_fin)

         EXTERNAL FunMul,MuL_d2sDISCC_dxdy,MuL_Q2d2sDISCC_dxdy,
     #                   MuL_d2sDISNC_dxdy
      ...
      endSELECT
         ...
         CALL MuLset(FunMuL,Res,RelErr,MinCal,2,*100)                    Intergation setup
...
*                  --------------------------------------------------- *
          endSELECT
            ...
            endSELECT
               ...
               mm_lep= m_lep**2
               DO n_W=1,Nm_fin
                 m_fin   = m_fin_ini+(n_W-1)*stepm_fin
                 mm_fin  = m_fin**2
                 E_nu_thr= ((m_fin+m_lep)**2-mm_ini)/(2*m_ini)
                 PRINT 204, n_FM_DIS,n,n_W,m_fin
                 DO n_E=1,NE_nu
                     ...
                     IF (n_E.eq.N_thr .or. n_E.eq.N_thr+1) THEN
                       ...
                       IF (dW2.lt.zero) THEN
                         ...
                         IF (Tables_sDISCC) THEN
                           CALL MuLInt(MuL_d2sDISCC_dxdy,  Res,*101)
                      endIF
                         IF (Tables_qDISCC) THEN
                           CALL MuLInt(MuL_Q2d2sDISCC_dxdy,Res,*101)
                      endIF
                    endIF
                  endIF
                endIF
                   IF (Tables_sDISCC) THEN
                     sDIS(n_E,n_W)= f*Res
                     PRINT 203, n_W,n_E, m_fin,E_nu,sDIS(n_E,n_W)
                endIF
                   IF (Tables_qDISCC) THEN
                     qDIS(n_E,n_W)= f*Res
                     PRINT 203, n_W,n_E, m_fin,E_nu,qDIS(n_E,n_W)
                endIF
              endDO
            endDO
               IF (Tables_sDISCC) THEN
     ...
     #           '/sDIS_'//R//'_'//Qc_DIS//
     #           '_'//BF_DIS//'_FIT_'//RT_DIS//'.dat')
                 DO n_E=1,NE_nu
                   WRITE(Ndat01,201)
     #             E_nu_out(n_E), (sDIS(n_E,n_W), n_W=1,Nm_fin)
              endDO
                 CLOSE(Ndat01)
            endIF
               IF (Tables_qDISCC) THEN
     ...
     #           '_FIT_'//RT_DIS//'.dat')
                 DO n_E=1,NE_nu
                   WRITE(Ndat01,201)
     #             E_nu_out(n_E), (qDIS(n_E,n_W), n_W=1,Nm_fin)
              endDO
                 CLOSE(Ndat01)
            endIF
                 ...
                 DO n_E=1,NE_nu
                   ...
                   IF (E_nu.le.E_nu_thr) THEN
                     ...
                     IF (dW2.lt.zero) THEN
                       Res= zero
                                      ELSE
                       CALL MuLInt(MuL_d2sDISNC_dxdy,Res,*101)
                  endIF
                endIF
                   sDIS(n_E,n_W)= f*Res
                   PRINT 203, n_W,n_E, m_fin,E_nu,sDIS(n_E,n_W)
...
*        ------------------------------------------------------------- *
         ...
         RETURN
  100    STOP 'ERROR IN MuLSet. SUBROUTINE sDIS_Tables'
  101    STOP 'ERROR IN MuLInt. SUBROUTINE sDIS_Tables'

  201 FORMAT(1PE11.5,4(1PE12.5))
  202 FORMAT(2x,'Q2_DIS =',F5.2,', A0_DIS =',F6.3,', B0_DIS =',F6.3,
     #', C0_DIS =',F6.3)
  203 FORMAT(2(2x,I3),2x,'m_fin =',F5.2,', E_nu =',1PE12.5,
     #', sDIS =',1PE12.5)
  204 FORMAT(2x,'n_FM_DIS =',I2,', n =',I2,', n_W =',I2,
     #', m_fin =',F5.2)
